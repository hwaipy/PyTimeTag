...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run a TimeTag app with Simulator source, split into DataBlocks, show counts with rich live table, and serialize blocks."
    )
    parser.add_argument("--source", default="simulator", choices=["simulator"], help="Data source type")
    ...
    parser.add_argument("--channel-count", type=int, default=8, help="Simulator channel count")
    ...
    console.print("[bold]TimeTag app started (Simulator source). Press Ctrl+C to stop.[/bold]")
    if args.save:
    ...
    with Live(latest_table, console=console, refresh_per_second=8) as live:
        ...
        for i in range(1, args.channel_count):
            sim.set_channel(i, mode="Random", random_count=50_000, threshold_voltage=-1.0, reference_pulse_v=1.0)

        channel_settings = _parse_channel_settings(args.channel, args.channel_count)
        for idx, cfg in channel_settings.items():
            sim.set_channel(idx, **cfg)
